-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/sanchay/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/sanchay/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/sanchay/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray,line:chararray);
...
rmf hdfs:///user/sanchay/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/sanchay/outputs' USING PigStorage('\t');
